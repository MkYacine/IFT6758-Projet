...
def clean_row(row):
    ...
    shots_and_goals = [
        play for play in all_plays_list if (play["result"]["event"] in ["Shot", "Goal"])
    ]
    for play in all_plays_list:
        if play["result"]["event"] in ["Shot", "Goal"]:
            ...
            time_elapsed = get_game_seconds(period, period_time) - get_game_seconds(
                last_period, last_period_time
            )
            play_data = {
                "period": period,
